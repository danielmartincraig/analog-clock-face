...
* [`src/cljs/analog_clock_face/`](src/cljs/analog_clock_face/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/analog_clock_face/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `analog-clock-face` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/analog_clock_face/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns analog-clock-face.example
  (:require [analog-clock-face.config :as config])
